...
import { Header1, Header2, Paragraph } from '../components/fonts'
...
const TalksPage = () => (
  <Layout noAuthor>
    <SEO title="Talks" />
    <Header1 marginBottom="zero">Talks</Header1>
    <Paragraph marginTop="s">
      These are some talks I have given over the years.
    </Paragraph>
    {talks.map(talk => (
      <Box key={talk.title}>
        <Header2>{talk.title}</Header2>
        <Paragraph>{format(talk.date, 'ddd, MMMM do yyyy')}</Paragraph>
        <iframe
          width="560"
          height="315"
